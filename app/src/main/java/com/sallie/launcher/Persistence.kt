...
class SalliePersistence(context: Context) {
    ...
    fun saveMood(mood: String) { prefs.edit().putString("mood", mood).apply() }
    fun loadMood(): String? = prefs.getString("mood", null)
    fun saveFatigue(f: Int) { prefs.edit().putInt("fatigue", f).apply() }
    ...
    fun saveTasks(ids: List<String>) { prefs.edit().putString("tasks", ids.joinToString(",")).apply() }
    fun loadTasks(): List<String> = prefs.getString("tasks", "")?.split(",")?.filter { it.isNotBlank() } ?: emptyList()
    fun saveConversationEntries(entries: List<ConversationEntry>) {
        val serialized = entries.joinToString("\n") { e ->
            listOf(e.timestamp.toString(), e.speaker.replace("\n"," "), e.text.replace("\n"," ")).joinToString("|")
        }
        prefs.edit().putString("conversation_entries", serialized).apply()
    }
    fun loadConversationEntries(): List<ConversationEntry> = prefs.getString("conversation_entries", null)
        ?.lineSequence()
        ?.mapNotNull { line ->
