...
class PlatformASR(
    ...
    override fun onReadyForSpeech(params: Bundle?) { onState("ready") }
    override fun onBeginningOfSpeech() { onState("speech-begin") }
    override fun onRmsChanged(rmsdB: Float) { onRms(rmsdB) }
    override fun onBufferReceived(buffer: ByteArray?) {}
    override fun onEndOfSpeech() { onState("speech-end") }
...
class PlatformTTS(app: Application, private val onState: (String) -> Unit) : TextToSpeech(app), TextToSpeech.OnInitListener {
    ...
    init { engine = TextToSpeech(application, this) }
    ...
    override fun onInit(status: Int) {
        if (status == TextToSpeech.SUCCESS) {
            ...
            onState("tts-ready")
        } else onState("tts-error:$status")
    }

    fun shutdownEngine() { engine?.shutdown(); engine = null }
}
